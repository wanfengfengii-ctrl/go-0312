...
func (s *Service) deduct(ctx context.Context, tx store.Store, req MaterialOperationRequest, disp material.Disposition, delta domain.Grams) error {
	s.packageBalanceMu.Lock()
	defer s.packageBalanceMu.Unlock()
	available, ok := s.packageBalanceSnapshots[req.PackageID]
	if !ok {
		...
		s.packageBalanceSnapshots[req.PackageID] = available
	}
	if available < domain.Grams(req.Grams) {
